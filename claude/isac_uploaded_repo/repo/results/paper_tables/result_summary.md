...
- Ablation: `ProposedV2_NoLoSGuard` causes the largest return drop (0.8926); in the current thresholds this behaves close to always-on predictive fallback, so the result says the guard/gating logic is essential for preserving throughput. Removing predictive fallback costs 0.0828 return and raises outage by 0.0112.
